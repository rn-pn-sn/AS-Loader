    ...
    public sealed class Mesh : NamedObject
    {
        ...
        public void ProcessData()
        {
            ...
            if (m_IndexBuffer.Length == 0)
            {
                ...
                Debug.LogWarning($"{msg} | PathID: {m_PathID} | Name: \"{m_Name}\"");
            }
            else
            {
                GetTriangles();
                        ...
                        if (version >= 2018)
                        {
                            switch (chn)
                            {
                                ...
                                case 13: //kShaderChannelBlendIndices
                                    ...
                                    break;
                                default:
                                    Debug.LogWarning($"Unknown vertex attribute: {chn}");
                                    break;
                            }
                        }
                        else
            ...
            if (version < 5)
            {
                if (m_CompressedMesh.m_BindPoses.m_NumItems > 0)
                {
                    ...
                    for (var i = 0; i < m_BindPose.Length; i++)
                    {
                        ...
                        matrix.m00 = m_BindPoses_Unpacked[startIndex + 0];
                        matrix.m10 = m_BindPoses_Unpacked[startIndex + 1];
                        matrix.m20 = m_BindPoses_Unpacked[startIndex + 2];
                        matrix.m30 = m_BindPoses_Unpacked[startIndex + 3];
                        matrix.m01 = m_BindPoses_Unpacked[startIndex + 4];
                        matrix.m11 = m_BindPoses_Unpacked[startIndex + 5];
                        matrix.m21 = m_BindPoses_Unpacked[startIndex + 6];
                        matrix.m31 = m_BindPoses_Unpacked[startIndex + 7];
                        matrix.m02 = m_BindPoses_Unpacked[startIndex + 8];
                        matrix.m12 = m_BindPoses_Unpacked[startIndex + 9];
                        matrix.m22 = m_BindPoses_Unpacked[startIndex + 10];
                        matrix.m32 = m_BindPoses_Unpacked[startIndex + 11];
                        matrix.m03 = m_BindPoses_Unpacked[startIndex + 12];
                        matrix.m13 = m_BindPoses_Unpacked[startIndex + 13];
                        matrix.m23 = m_BindPoses_Unpacked[startIndex + 14];
                        matrix.m33 = m_BindPoses_Unpacked[startIndex + 15];
                        m_BindPose[i] = matrix;
                    }
                }
            }
            ...
            for (var i = 0; i < len; i++)
            {
                switch (format)
                {
                    case VertexFormat.Float:
                        ...
                        break;
                    case VertexFormat.Float16:
                        ushort halfValue = BinaryPrimitives.ReadUInt16LittleEndian(inputBytes.AsSpan(i * 2));
                        result[i] = (float)new half(halfValue);
                        break;
                    case VertexFormat.UNorm8:
